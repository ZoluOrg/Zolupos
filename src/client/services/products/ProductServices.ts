...
import { IRestockRequest } from "../../interfaces/inventory/IRestockRequest";
import { IResultWrapper } from "../../wrapper/ResultWrapper";

const token = Cookies.get("zoluken");

export const getProducts = async () => {
  let request = await axios.get<IResultWrapper<Array<IProduct>>>(
...
export const getProductsById = async (id: number) => {
  let request = await axios.get<IResultWrapper<IProduct>>(
...
export const addProduct = async (productToAdd: IAddProductRequest) => {
  let request = await axios.post<IResultWrapper<number>>(
    "https://localhost:7116/api/Inventory/",
    {
      productToAdd,
      headers: { Authorization: `Bearer ${token}` },
...
export const editProduct = async (editedProduct: IProduct) => {
  let request = await axios.patch<IResultWrapper<IProduct>>(
...
export const restockProduct = async (restockRequest: IRestockRequest) => {
  let request = await axios.patch<IResultWrapper<number>>(
